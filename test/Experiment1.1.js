...
const PriceFromContractsOraclePlusRandomization=artifacts.require('PriceFromContractsOraclePlusRandomization')
...
contract('Experiemnt1.1', function (accounts) {
    ...
    beforeEach(async () => {
        ...
        //Creation of Conditional Tokens with a CategoricalEvent
	       secretnr=89;
	        secret='764761E297D6f121e79C32A65829Cd1dDb4D89';
          //web3.utils.soliditySha3(param1 [, param2, ...]) is currenlty not working
          //ecretEncrypted=web3.utils.soliditySha3(secretnr,secret);
          //secretEncrypted=sha3(secret);
          secretEncrypted='0x4348a1b71b6d2ead36223cfc497cb0a25298da46b09bc20f08d24addb9b2f466';


       avgPriceOracleForCategoricalEvent = await PriceFromContractsOraclePlusRandomization.new(targetBlock,secretEncrypted);
    ...
    it('should test futarchy oracle with successful manipulation', async () => {
       ...
       assert.equal(await OutcomeToken.at(await difficultyPlusFivePercentEvent.outcomeTokens(0)).balanceOf(accounts[manipulator]),tokenCountManipulator1Short);
        ...
        await difficultyOracle.setOutcome((currentDifficulty+randomNr));
        await difficultyPlusFivePercentOracle.setOutcome(currentDifficulty+randomNr);
        await avgPriceOracleForCategoricalEvent.setOutcome(secretnr,secret);
        ...
        if(await avgPriceOracleForCategoricalEvent.getOutcome()==0){
  ...
  it('should test futarchy oracle non-manipulated', async () => {
      ...
      console.log("normal difficulty market gives:" + (await normalDifficultyMarket.getAvgPrice.call()).valueOf())
      console.log(" difficultyPlusFivePercent market gives:" + (await difficultyPlusFivePercentMarket.getAvgPrice.call()).valueOf())
      if(await avgPriceOracleForCategoricalEvent.getOutcome()==1){
